...
public class SoundEngine {
	...
	private synchronized void loadLibrary() {
		if (!this.loaded) {
			try {
				...
				this.listener.reset();
				this.listener.setGain(this.options.getSoundVolume(SoundCategory.MASTER));
	...
	private float getVolume(@Nullable SoundCategory category) {
		return category != null && category != SoundCategory.MASTER ? this.options.getSoundVolume(category) : 1.0F;
	}
	...
	private void tickNonPaused() {
		this.tickCount++;
		this.queuedTickableSounds.stream().filter(SoundInstance::canPlay).forEach(this::play);
		this.queuedTickableSounds.clear();
		...
		while (iterator.hasNext()) {
			...
			float h = this.options.getSoundVolume(soundInstance.getCategory());
			if (h <= 0.0F) {
				...
				if (i <= this.tickCount) {
					if (shouldLoopManually(soundInstance)) {
						this.queuedSounds.put(soundInstance, this.tickCount + soundInstance.getRepeatDelay());
					}
					...
					try {
						this.instanceBySource.remove(soundInstance.getCategory(), soundInstance);
	...
	private static boolean shouldLoopManually(SoundInstance sound) {
		return sound.isRepeatable() && requiresManualLooping(sound);
	}
	...
	public void play(SoundInstance sound) {
		if (this.loaded) {
			if (sound.canPlay()) {
				WeightedSoundSet weighedSoundEvents = sound.getSoundSet(soundManager);
				...
				if (weighedSoundEvents == null) {
					...
					if (sound2 != null) {
						if (sound2 == SoundManager.MISSING_SOUND) {
							...
							SoundInstance.AttenuationType attenuation = sound.getAttenuationType();
							...
							if (h == 0.0F && !sound.shouldAlwaysPlay()) {
								...
								if (this.listener.getGain() <= 0.0F) {
									...
									CompletableFuture<ChannelAccess.ChannelHandle> completableFuture = this.channelAccess
										.createHandle(sound2.isStreamed() ? Library.Pool.STREAMING : Library.Pool.STATIC);
	...
	private float calculateVolume(SoundInstance sound) {
		return this.calculateVolume(sound.getVolume(), sound.getCategory());
	}
	...
	public void stop(@Nullable Identifier soundName, @Nullable SoundCategory category) {
		if (category != null) {
			for (SoundInstance soundInstance : this.instanceBySource.get(category)) {
				if (soundName == null || soundInstance.getId().equals(soundName)) {
			...
			for (SoundInstance soundInstancex : this.instanceToChannel.keySet()) {
				if (soundInstancex.getId().equals(soundName)) {
